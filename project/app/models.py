...
class Usuario(SafeDeleteModel, AbstractUser):
    ...
    endereco = models.CharField(
        'Endereço', max_length=200)
    cpf = models.CharField(
        max_length=30, verbose_name='CPF',
        validators=[
            RegexValidator(
                r'^\d{3}\.\d{3}\.\d{3}\-\d{2}$',
                message="Formato esperado: '999.999.999-99'")])
    rg = models.CharField(
        max_length=30, verbose_name='RG',
        validators=[
            RegexValidator(
                r'^(\d\.?-?)+$')])
...
class Pedido(SafeDeleteModel):
    ...
    endereco = models.CharField(
        'Endereço', max_length=200)
    feito = models.DateTimeField(
        'Feito em', auto_now_add=True)
